...
// 		$img = imagecreatetruecolor( $this->max_img_width  , $this->max_img_height );
	...
	private function draw_node( node $node ){
		...
		$layout->do_layout();
		$x = ( ( $node->get_relcol()  ) * $this->max_img_width * $this->width_margin ) + ($this->max_node_width_px * $this->width_margin /2);
		$y = ( ( $node->get_relrow()  ) * $this->max_img_height * $this->height_margin ) + ($this->max_node_height_px * $this->height_margin /2);
