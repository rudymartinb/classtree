...
// 		imageantialias ( $img, true );
	...
	private function white_arrow( int $x1, int $y1, int $x2, int $y2, int $arrow_type = 1 ) {
		 ...
		 */
		$distance = 15;
		$point1 = $this->calculate_point($x10, $y1, $x2, $y2, $distance, $alpha1 );
		$xx1 = $point1["x"];
		$yy1 = $point1["y"];
		
		$point1 = $this->calculate_point($x10, $y1, $x2, $y2, $distance, $alpha2 );
		$xx2 = $point1["x"];
		$yy2 = $point1["y"];
		
		// draw the line
		imageline ( $this->img , $x10 , $y10 , $x2 , $y2 , $this->color["black"] );
