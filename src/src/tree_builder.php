...
// 		var_dump( "CX" , $this->get_max_width(), $this->get_max_height()  );
		...
		$this->max_img_width = ( $this->get_max_columns() +1 ) * $this->max_node_width_px  * $this->width_margin ;
		$this->max_img_height = ( $this->get_max_height()+1) * $this->max_node_height_px * $this->height_margin ;
...
// 		$img = imagecreatetruecolor( 1000 , 800 );
		...
		$this->color["red" ] = imagecolorallocate($img, 255,   0,  0);
...
// 		imagefilledrectangle( $img, 0,0,1000, 800, $this->color["white"]);
	...
	private function draw_node( node $node ){
		...
		$layout->do_layout();
		$x = ( ( $node->get_relcol()+($node->get_width()/2) ) * $this->max_node_width_px  * $this->width_margin  ) ;
		$y = ( ( $node->get_relrow()  ) * $this->max_node_height_px * $this->height_margin ) + ($this->max_node_height_px* $this->height_margin / 2 );
